...
class MemoryManager:
    ...
    Coordinates short-term message history, medical entity tracking, and summarization
    ...
    def __init__(
        ...
        summary_memory: Optional[SummaryMemory] = None,
    ):
        ...
        sum_enabled = m_cfg.get("summary", {}).get("enabled", True) and self.enabled
        sum_trigger = int(m_cfg.get("summary", {}).get("trigger_messages", 10))
        ...
        self.summary_memory = summary_memory or SummaryMemory(llm=llm, trigger_messages=sum_trigger, enabled=sum_enabled)
    ...
    def add_message(self, conversation_id: str, role: str, content: str) -> None:
        """
        Appends a message to the conversation history and updates entity & summary memory.
        """
        ...
        # 3. Check and update summary if trigger limit reached
        recent = self.short_term.get_recent_messages(conversation_id)
        self.summary_memory.update_summary(conversation_id, recent)

    def get_context(self, conversation_id: Optional[str]) -> MemoryContext:
        """
        Returns structured MemoryContext (recent messages, entities, summary) for given conversation_id.
        Returns empty MemoryContext if conversation_id is None or memory is disabled.
        """
        if not self.enabled or not conversation_id:
            return MemoryContext(recent_messages=[], entities={}, summary="")
    ...
    def clear_conversation(self, conversation_id: str) -> None:
        """
        Clears all short-term history, entity memory, and summary for given conversation_id.
        """
        ...
        self.summary_memory.clear(conversation_id)
